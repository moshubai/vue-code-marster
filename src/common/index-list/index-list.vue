  ...
  <div class="index-list" ref="indexlist" :style="{ 'height': currentHeight + 'px', 'padding-right': navWidth + 'px'}">
    ...
    <div class="index-list-nav" @touchstart="handleTouchStart" ref="nav">
      <ul class="index-list-navlist">
        <li class="index-list-navitem" v-for="section in sections">
          {{ section.index }}
          </li>
      </ul>
    </div>
...
export default {
  ...
  methods: {
    init() {
      ...
      let listItems = this.$refs.indexlist.getElementsByClassName(
        "index-list-item"
      );
    ...
    scrollList(y) {
      ...
      if (
        !currentItem ||
        !currentItem.classList.contains("index-list-navitem")
      ) {
        return;
      }
  ...
  mounted() {
    if (!this.currentHeight) {
      this.currentHeight =
        ...
        this.$refs.indexlist.getBoundingClientRect().top;
    }
    this.init();
  }
};
